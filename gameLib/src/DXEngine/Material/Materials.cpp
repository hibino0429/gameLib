...
//-----------------------------------------------------------------------------
void	Materials::Load(const std::string& filePath)
{
	//ファイル読み込み
...
//-----------------------------------------------------------------------------
void	Materials::Create(const std::string& shaderSource)
{
	...
	if (!vertexShader->Create("./data/shaderData/textureShader", inputElementDesc.data(), inputElementDesc.size()))
	{
		MessageBoxA(nullptr, "失敗", "頂点シェーダ", MB_OK);
	}
	if (!pixelShader->Create("./data/shaderData/textureShader"))
	{
...
//-----------------------------------------------------------------------------
void	Materials::SetConstantBuffer(const int slot, void* constantBuf, size_t size)
{
	...
	{
		//エラー
		MessageBoxA(nullptr, "コンスタントバッファの作成に失敗", "Materials::SetConstantBuffer()",MB_OK);
	}
}
...
//-----------------------------------------------------------------------------
void	Materials::SetTexture(const int slot, Texture* texture)
{
